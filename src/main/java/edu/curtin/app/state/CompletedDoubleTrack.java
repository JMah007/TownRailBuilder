...
public class CompletedDoubleTrack implements RailState{
    public static final int MAX_NUM_GOODS_TRANSPORTED = 200; // the maximum number of goods that can be transported by 2 trains per day

    private static final Logger logger = Logger.getLogger(CompletedDoubleTrack.class.getName());
    ...
    @Override
     /**
     * This method is the concrete method that simulates the transport of goods for a single track. It can move 200 goods per day max  as it has 2 rails so it deducts this from num of goods stocked in the town and adds this to num ofgppds transported for the town
    ...
    public void transportGoodsHandler(Railway rail, Map<String, Town> townList) {
        ...
        if(rail.getDirectionAlternator() == 0){ // Simulate goods being transported from town1 as 0 means train is currently at town1
            ...
            if (numGoodsStockedTown1 <= MAX_NUM_GOODS_TRANSPORTED){
                town1.setNumGoodsStocked(0); // since the train can always take max of 200 goods per day, we set the stocks back to 0 if less than 200 as all of it has been taken
                town1.setNumGoodsTransported(numGoodsStockedTown1); // The amount of goods taken away gets added to the total amount of goods transported by the town
            }
            else{
                town1.setNumGoodsStocked(numGoodsStockedTown1 - MAX_NUM_GOODS_TRANSPORTED); // number of goods left is more than 200 so we deduct 100 goods from the stockpile
            ...
            if (numGoodsStockedTown2 <= MAX_NUM_GOODS_TRANSPORTED){
                town2.setNumGoodsStocked(0); // since the train can always take max of 200 goods per day, we set the stocks back to 0 if less than 200 as all of it has been taken
                town2.setNumGoodsTransported(numGoodsStockedTown2); // The amount of goods taken away gets added to the total amount of goods transported by the town
            }
            else{
                town2.setNumGoodsStocked(numGoodsStockedTown2 - MAX_NUM_GOODS_TRANSPORTED); // number of goods left is more than 200 so we deduct 200 goods from the stockpile
     ...
     * @return a string that says "Completed Double Track"
     */
    public String getCurrentState() {
        return "Completed Double Track";
    }
}
